...
export const BUILTIN_UI_ITEMS: BuiltinUiItem[] = [
	...
	},
	{
		...
		align: "end",
		/* 展示型（版本号 + 更新红点），缺省收进「⋯」。
		   与 sound/language/theme 不同：更新界面整块搬进了设置面板的「更新」页
		   （低频操作 + 需要解释的东西太多，塞进 40px 的下拉放不下），所以 ⋯ 菜单里
		   这一项不再是完整下拉，而是带路到设置「更新」页的薄入口（TopBar 的
		   host:update 节点工厂）。想把它放回顶栏常驻的用户可在「设置 → 布局」里勾回来
		   —— 节点工厂保留着，否则勾回来也画不出东西。 */
		hidden: true,
	},
